...
export default function ChangePasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Force a session update to clear the must_change_password flag client-side
            await update({ must_change_password: false });

            // Redirect will be handled by middleware or by router based on role
            router.push("/dashboard/" + (session?.user?.role?.toLowerCase() || "login"));
            router.refresh();
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900">
            <div className="w-full max-w-md mx-4">
                <div className="bg-white/10 backdrop-blur-xl border border-white/10 rounded-2xl p-8 shadow-2xl">
                    <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-orange-500/20 text-orange-400 mb-6 mx-auto">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div>
                            <label className="block text-xs font-medium text-blue-200/80 mb-1.5">
                                New Password *
                            </label>
                            <input
                                type="password"
                        ...
                        <button
                            ...
                            className="w-full h-11 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 text-white font-medium text-sm rounded-lg transition-all duration-200 flex items-center justify-center shadow-lg shadow-blue-600/25 mt-2"
                        >
