        ...
        <div className="space-y-6 max-w-5xl mx-auto pb-10">
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">{order.buyer.name} &bull; {order.order_type}</p>
                </div>
                <span className={`px-3 py-1.5 text-xs font-bold uppercase rounded-md ${STATUS_COLORS[order.status] || "bg-gray-100 text-gray-800"}`}>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden flex flex-col sm:flex-row">
                        <div className="flex-1 p-5 border-b sm:border-b-0 sm:border-r border-gray-100 bg-slate-50">
                            <p className="text-xs text-gray-500 uppercase tracking-wider font-bold mb-1">Order Value</p>
                            <p className="text-2xl font-bold text-gray-900">₹{order.total_amount.toLocaleString("en-IN", { minimumFractionDigits: 2 })}</p>
                        </div>
                        <div className="flex-1 p-5 bg-orange-50/30">
                            ...
                            <p className="text-xs text-gray-500 mt-1">{order.expenses.length} expense request(s)</p>
                        </div>
                    </div>

                    {/* Order Info */}
                    <div className="bg-white rounded-xl border border-gray-200 p-5">
                        <h3 className="text-sm font-bold text-gray-900 mb-4 uppercase tracking-wide border-b pb-2">Order Details</h3>
                ...
                <div className="space-y-6">
                    <div className="bg-white rounded-xl border border-gray-200 p-4">
                        ...
                        <div className="space-y-3">
                            <div><p className="text-xs text-gray-500">Merchandiser</p><p className="text-sm font-medium">{order.merchandiser?.name || "—"}</p></div>
                            <div><p className="text-xs text-gray-500">{order.order_type === "SAMPLE" ? "Sample PM" : "Production PM"}</p><p className="text-sm font-medium">{(order.order_type === "SAMPLE" ? order.assigned_sample_pm?.name : order.assigned_production_pm?.name) || "—"}</p></div>
                        </div>
                    </div>
                    <div className="bg-white rounded-xl border border-gray-200 p-4">
