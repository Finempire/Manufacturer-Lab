                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-5">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                            ...
                            <div><p className="text-xs text-gray-500 mb-1">Buyer</p><p className="text-sm font-medium">{order.buyer.name}</p></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-5">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-sm font-bold text-gray-900">Tech Packs ({order.tech_packs.length})</h3>
                            </div>
                            {canCreateTechPack && (
                        ...
                        {order.tech_packs.length === 0 ? (
                            <p className="text-sm text-gray-400 text-center py-6">No tech packs yet. Create one to start working.</p>
                        ) : (
                            <div className="space-y-3">
                                {order.tech_packs.map(tp => (
                                    <div key={tp.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100 hover:bg-gray-100 transition cursor-pointer">
                                        <div>
                                            <p className="text-sm font-medium text-gray-900">{tp.tech_pack_no}</p>
                                            <p className="text-xs text-gray-400 mt-0.5">{tp.fabric_details || "No fabric details"}</p>
                                        </div>
