...
export default function ProductionExpensesPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            const payload = { ...formData, expected_amount: parseFloat(formData.expected_amount) };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">Raise and track production expense requests</p>
                </div>
                <button
                    onClick={openDrawer}
            ...
            <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${drawerOpen ? "translate-x-0" : "translate-x-full"}`}>
                <div className="flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-y-auto px-6 py-5">
                        <form id="new-expense-form-prod" onSubmit={handleSubmit} className="space-y-5">
                            <div>
                    ...
                    <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
                        ...
                        <button type="submit" form="new-expense-form-prod" disabled={submitting} className="px-6 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50">{submitting ? "Submitting..." : "Submit Request"}</button>
                    </div>
                </div>
            </div>
