            ...
            <form onSubmit={handleSubmit} className="bg-surface-1 rounded-lg border border-border p-6 space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" required className="w-full h-10 px-3 border border-border rounded-lg text-sm" value={form.required_by_date} onChange={e => setForm({ ...form, required_by_date: e.target.value })} />
                    </div>
                </div>
                    ...
                    <div className="space-y-2">
                        {lines.map((line, i) => (
                            <div key={i} className="grid grid-cols-12 gap-2 items-center">
                                <input type="text" required placeholder="Material name *" className="col-span-4 h-9 px-3 border border-border rounded-lg text-sm" value={line.material_name} onChange={e => handleLineChange(i, "material_name", e.target.value)} />
