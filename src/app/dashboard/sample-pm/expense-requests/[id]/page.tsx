...
export default function SamplePMExpenseDetailPage() {
    ...
    if (!expense) return <div className="text-center py-12 text-gray-400">Not found</div>;
    ...
    return (
        <div className="space-y-6 max-w-3xl mx-auto pb-10">
            <div className="flex items-center gap-4">
                ...
                <span className={`px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-md border ${STATUS_COLORS[expense.status] || "bg-gray-100 text-gray-800 border-gray-200"}`}>{expense.status.replace(/_/g, " ")}</span>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-6">
                    ...
                    <div><p className="text-xs text-gray-500 mb-1">Expected</p><p className="text-sm font-bold">₹{expense.expected_amount.toLocaleString("en-IN")}</p></div>
                    {expense.actual_amount && <div><p className="text-xs text-gray-500 mb-1">Paid</p><p className="text-sm font-bold text-green-700">₹{expense.actual_amount.toLocaleString("en-IN")}</p></div>}
            ...
            {expense.attachment_path && (
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                    ...
                    <button onClick={() => window.open(`/api/files/${expense.attachment_path}?action=inline`, '_blank')} className="px-4 py-2 text-xs font-medium bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100">View</button>
                </div>
            )}
        </div>
    );
