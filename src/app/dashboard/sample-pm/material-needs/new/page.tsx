    ...
    return (
        <div className="max-w-3xl mx-auto pb-10">
            <div className="flex items-center gap-4 mb-6">
                <button onClick={() => router.back()} className="p-2 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition"><ArrowLeft className="w-5 h-5 text-slate-600" /></button>
                <h1 className="text-lg font-semibold tracking-tight text-slate-900">New Material Requirement</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-slate-200 p-6 space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select required className="w-full h-10 px-3 border border-slate-300 rounded-lg text-sm" value={form.style_id} onChange={e => setForm({ ...form, style_id: e.target.value })}>
                ...
                <div>
                    ...
                    <input type="date" required className="w-full h-10 px-3 border border-slate-300 rounded-lg text-sm max-w-xs" value={form.required_by_date} onChange={e => setForm({ ...form, required_by_date: e.target.value })} />
                </div>

                <div>
                    <label className="block text-xs font-medium text-slate-700 mb-1">Special Instructions</label>
                    <textarea className="w-full p-3 border border-slate-300 rounded-lg text-sm" rows={2} value={form.special_instructions} onChange={e => setForm({ ...form, special_instructions: e.target.value })} />
                </div>
                ...
                <div className="space-y-3">
                    ...
                    {lines.map((line, i) => (
                        <div key={i} className="grid grid-cols-12 gap-2 items-center">
                            <input type="text" required placeholder="Material *" className="col-span-4 h-9 px-3 border border-slate-300 rounded-lg text-sm" value={line.material_name} onChange={e => handleLineChange(i, "material_name", e.target.value)} />
                            <input type="text" placeholder="Description" className="col-span-3 h-9 px-3 border border-slate-300 rounded-lg text-sm" value={line.description} onChange={e => handleLineChange(i, "description", e.target.value)} />
                            <input type="number" required min="0.01" step="0.01" placeholder="Qty" className="col-span-2 h-9 px-3 border border-slate-300 rounded-lg text-sm" value={line.quantity} onChange={e => handleLineChange(i, "quantity", e.target.value)} />
                            <select className="col-span-2 h-9 px-2 border border-slate-300 rounded-lg text-sm" value={line.unit} onChange={e => handleLineChange(i, "unit", e.target.value)}>
