        ...
        <div className="space-y-6 max-w-4xl mx-auto pb-10">
            <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">{request.order.order_no} &bull; {request.buyer.name}</p>
                </div>
                <span className={`px-3 py-1.5 text-xs font-bold uppercase rounded-md ${STATUS_COLORS[request.status] || "bg-gray-100 text-gray-800"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 space-y-6">
                    <div className="bg-white rounded-xl border border-gray-200 p-5">
                        <h3 className="text-sm font-bold text-gray-900 mb-4 uppercase tracking-wide border-b pb-2">Request Details</h3>
                        <div className="grid grid-cols-2 gap-y-4 gap-x-6">
                            <div><p className="text-xs text-gray-500 mb-1">Runner</p><p className="text-sm font-medium">{request.runner.name}</p></div>
                            <div><p className="text-xs text-gray-500 mb-1">Preferred Vendor</p><p className="text-sm font-medium">{request.preferred_vendor?.name || "—"}</p></div>
                            {request.style && <div><p className="text-xs text-gray-500 mb-1">Style</p><p className="text-sm font-medium">{request.style.style_code}</p></div>}
                            {request.expected_date && <div><p className="text-xs text-gray-500 mb-1">Expected By</p><p className="text-sm font-medium">{format(new Date(request.expected_date), "dd MMM yyyy")}</p></div>}
                            {request.store_location && <div><p className="text-xs text-gray-500 mb-1">Store Location</p><p className="text-sm font-medium">{request.store_location}</p></div>}
                            {request.remarks && <div className="col-span-2"><p className="text-xs text-gray-500 mb-1">Remarks</p><p className="text-sm text-gray-700">{request.remarks}</p></div>}
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                        <div className="px-5 py-3 border-b border-gray-100 bg-gray-50">
                            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Material Lines</h3>
                        </div>
                        <table className="min-w-full divide-y divide-gray-100">
                            <thead className="bg-gray-50/50">
                                <tr>
                                    <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 uppercase">Material</th>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {request.lines.map(line => (
                                    <tr key={line.id}>
                                        <td className="px-4 py-2.5 text-sm text-gray-900">{line.material.description}</td>
                    ...
                    {request.purchases.length > 0 && (
                        <div className="bg-white rounded-xl border border-gray-200 p-4">
                            <h4 className="text-sm font-bold text-gray-900 mb-3">Purchases ({request.purchases.length})</h4>
                            <div className="space-y-2">
                                {request.purchases.map(p => (
                                    <div key={p.id} className="flex items-center justify-between py-1.5 px-2 bg-gray-50 rounded-lg">
                                        <span className="text-xs font-medium text-gray-700">{p.purchase_no}</span>
                ...
                <div>
                    <div className="bg-white rounded-xl border border-gray-200 p-4">
                        ...
                        <div className="space-y-3">
                            <div><p className="text-xs text-gray-500">Total Materials</p><p className="text-sm font-bold">{request.lines.length} items</p></div>
                            <div><p className="text-xs text-gray-500">Expected Total</p><p className="text-sm font-bold text-gray-900">₹{totalExpected.toLocaleString("en-IN")}</p></div>
                            <div><p className="text-xs text-gray-500">Created</p><p className="text-sm font-medium">{format(new Date(request.created_at), "dd MMM yyyy")}</p></div>
                        </div>
                    </div>
                </div>
            </div>
