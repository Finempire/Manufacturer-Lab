    ...
    return (
        <div className="max-w-3xl mx-auto pb-10">
            <div className="flex items-center gap-4 mb-6">
                ...
                <h1 className="text-lg font-semibold tracking-tight text-foreground">New Material Request</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-surface-1 rounded-lg border border-border p-6 space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-xs font-medium text-foreground-secondary mb-1">Assign Runner *</label>
                        <select required className="w-full h-10 px-3 border border-border rounded-lg text-sm" value={form.assigned_runner_id} onChange={e => setForm({ ...form, assigned_runner_id: e.target.value })}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label className="block text-xs font-medium text-foreground-secondary mb-1">Preferred Vendor</label>
                        <select className="w-full h-10 px-3 border border-border rounded-lg text-sm" value={form.preferred_vendor_id} onChange={e => setForm({ ...form, preferred_vendor_id: e.target.value })}>
                    ...
                    <div>
                        ...
                        <input type="date" className="w-full h-10 px-3 border border-border rounded-lg text-sm" value={form.expected_date} onChange={e => setForm({ ...form, expected_date: e.target.value })} />
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-foreground-secondary mb-1">Store Location</label>
                        <input type="text" className="w-full h-10 px-3 border border-border rounded-lg text-sm" value={form.store_location} onChange={e => setForm({ ...form, store_location: e.target.value })} />
                    </div>
                </div>

                <div>
                    <label className="block text-xs font-medium text-foreground-secondary mb-1">Remarks</label>
                    <textarea className="w-full p-3 border border-border rounded-lg text-sm" rows={2} value={form.remarks} onChange={e => setForm({ ...form, remarks: e.target.value })} />
                </div>
                                ...
                                {lines.map((line, i) => (
                                    <tr key={i}>
                                        <td className="px-3 py-2">
                                            <select required className="w-full h-8 px-2 border border-border rounded text-sm" value={line.material_id} onChange={e => handleMaterialSelect(i, e.target.value)}>
                                                <option value="">Select Material</option>
                                                {materials.map(m => <option key={m.id} value={m.id}>{m.description} ({m.category})</option>)}
                                            </select>
                                        </td>
                                        <td className="px-3 py-2"><input type="number" required min="0.01" step="0.01" className="w-20 h-8 px-2 border border-border rounded text-sm text-right" value={line.quantity} onChange={e => handleLineChange(i, "quantity", e.target.value)} /></td>
                                        <td className="px-3 py-2"><input type="number" min="0" step="0.01" className="w-24 h-8 px-2 border border-border rounded text-sm text-right" value={line.expected_rate} onChange={e => handleLineChange(i, "expected_rate", e.target.value)} /></td>
