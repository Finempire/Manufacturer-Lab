    ...
    return (
        <div className="max-w-2xl mx-auto pb-10">
            <div className="flex items-center gap-4 mb-6">
                ...
                <h1 className="text-xl font-bold text-gray-900">New Expense Request</h1>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select required className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.buyer_id} onChange={e => setForm({ ...form, buyer_id: e.target.value })}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select required className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.expense_category} onChange={e => setForm({ ...form, expense_category: e.target.value })}>
                            {CATEGORIES.map(c => <option key={c} value={c}>{c.replace(/_/g, " ")}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Expense Date *</label>
                        <input type="date" required className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.expense_date} onChange={e => setForm({ ...form, expense_date: e.target.value })} />
                    </div>
                    <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Expected Amount *</label>
                        <input type="number" step="0.01" required className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" placeholder="₹0.00" value={form.expected_amount} onChange={e => setForm({ ...form, expected_amount: e.target.value })} />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.style_id} onChange={e => setForm({ ...form, style_id: e.target.value })}>
                    ...
                    <div>
                        ...
                        <input type="text" className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.vendor_name} onChange={e => setForm({ ...form, vendor_name: e.target.value })} />
                    </div>
                </div>

                {form.expense_category === "JOB_WORK" && (
                    <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Job Work Type</label>
                        <input type="text" className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" placeholder="e.g. Embroidery, Printing" value={form.job_work_type} onChange={e => setForm({ ...form, job_work_type: e.target.value })} />
                    </div>
                )}

                <div>
                    <label className="block text-xs font-medium text-gray-700 mb-1">Description *</label>
                    <textarea required className="w-full p-3 border border-gray-300 rounded-lg text-sm" rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
                </div>
