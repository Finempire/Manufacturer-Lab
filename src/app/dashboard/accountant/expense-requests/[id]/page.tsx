                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-lg border border-slate-200 p-5">
                        ...
                        <div className="grid grid-cols-2 gap-y-4 gap-x-6">
                            ...
                            {expense.style && <div><p className="text-xs text-slate-500 mb-1">Style</p><p className="text-sm font-medium">{expense.style.style_code}</p></div>}
                            {expense.job_work_type && <div><p className="text-xs text-slate-500 mb-1">Job Work Type</p><p className="text-sm font-medium">{expense.job_work_type}</p></div>}
                    ...
                    <div className="bg-white rounded-lg border border-slate-200 overflow-hidden flex flex-col sm:flex-row">
                        <div className="flex-1 p-5 border-b sm:border-b-0 sm:border-r border-slate-100 bg-slate-50">
                            <p className="text-xs text-slate-500 uppercase tracking-wider font-bold mb-1">Expected Amount</p>
                            <p className="text-2xl font-bold text-slate-900">₹{expense.expected_amount.toLocaleString("en-IN")}</p>
                        </div>
                        <div className="flex-1 p-5 bg-blue-50/30">
                            ...
                            {expense.payment_method && <p className="text-xs text-slate-500 mt-1">Method: {expense.payment_method}</p>}
                        </div>
                    </div>
                    ...
                    {(expense.attachment_path || expense.payment_proof_path) && (
                        <div className="bg-white rounded-lg border border-slate-200 p-5">
                            <h3 className="text-sm font-bold text-slate-900 mb-4 uppercase tracking-wide">Documents</h3>
                            <div className="grid grid-cols-2 gap-4">
                                {expense.attachment_path && (
                                    <div className="border border-slate-200 rounded-lg p-3">
                                        ...
                                        <button onClick={() => window.open(`/api/files/${expense.attachment_path}?action=inline`, '_blank')} className="w-full py-1.5 text-xs font-medium bg-slate-50 border border-slate-200 rounded hover:bg-slate-100">View</button>
                                    </div>
                                )}
                                {expense.payment_proof_path && (
                                    <div className="border border-slate-200 rounded-lg p-3">
                                        ...
                                        <button onClick={() => window.open(`/api/files/${expense.payment_proof_path}?action=inline`, '_blank')} className="w-full py-1.5 text-xs font-medium bg-slate-50 border border-slate-200 rounded hover:bg-slate-100">View</button>
                                    </div>
                                )}
                            </div>
                        </div>
                ...
                <div className="space-y-4">
                    <div className="bg-white rounded-lg border border-slate-200 p-4 space-y-3">
                        ...
                        {action === "approve" && (
                            <div className="space-y-2">
                                <textarea placeholder="Notes (optional)" className="w-full p-2.5 border border-slate-300 rounded-lg text-sm" rows={2} value={notes} onChange={e => setNotes(e.target.value)} />
                                <div className="flex gap-2">
                                    <button onClick={() => setAction(null)} className="flex-1 py-2 text-sm text-slate-600 bg-slate-100 rounded-lg">Cancel</button>
                        ...
                        {action === "reject" && (
                            <div className="space-y-2">
                                <textarea placeholder="Rejection reason *" className="w-full p-2.5 border border-slate-300 rounded-lg text-sm" rows={2} value={rejectReason} onChange={e => setRejectReason(e.target.value)} />
                                <div className="flex gap-2">
                                    <button onClick={() => setAction(null)} className="flex-1 py-2 text-sm text-slate-600 bg-slate-100 rounded-lg">Cancel</button>
                        ...
                        {action === "pay" && (
                            <div className="space-y-3">
                                <div>
                                    <label className="block text-xs font-medium text-slate-700 mb-1">Actual Amount</label>
                                    <input type="number" step="0.01" className="w-full h-9 px-3 border border-slate-300 rounded-lg text-sm" value={actualAmount} onChange={e => setActualAmount(e.target.value)} />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-slate-700 mb-1">Payment Method</label>
                                    <select className="w-full h-9 px-3 border border-slate-300 rounded-lg text-sm" value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)}>
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-slate-700 mb-1">Reference ID</label>
                                    <input type="text" className="w-full h-9 px-3 border border-slate-300 rounded-lg text-sm" value={paymentRef} onChange={e => setPaymentRef(e.target.value)} />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-slate-700 mb-1">Payment Proof</label>
                    ...
                    {expense.accountant_notes && (
                        <div className="bg-white rounded-lg border border-slate-200 p-4">
                            <h4 className="text-sm font-bold text-slate-900 mb-2">Accountant Notes</h4>
                            <p className="text-sm text-slate-600">{expense.accountant_notes}</p>
                        </div>
                    )}
                </div>
            </div>
