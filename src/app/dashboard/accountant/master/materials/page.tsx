        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">{materials.length} materials</p>
                </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {materials.map((m) => (
                            <tr key={m.id} className="hover:bg-gray-50">
                                <td className="px-4 py-3 text-sm font-mono text-gray-500">{m.sku_code || "—"}</td>
                                <td className="px-4 py-3 text-sm font-medium text-gray-900">{m.description}</td>
                                <td className="px-4 py-3"><span className="px-2.5 py-1 text-[11px] font-semibold rounded-full bg-gray-100 text-gray-700">{m.category}</span></td>
                                <td className="px-4 py-3 text-sm text-gray-500">{m.unit_of_measure}</td>
                                <td className="px-4 py-3 text-sm text-gray-900 text-right tabular-nums">{m.default_rate ? `₹${m.default_rate.toFixed(2)}` : "—"}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.sku_code} onChange={(e) => setForm({ ...form, sku_code: e.target.value })} />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-gray-700 mb-1">Category *</label>
                                    <select className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })}>
                            ...
                            <div>
                                ...
                                <input type="text" required className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-medium text-gray-700 mb-1">Unit of Measure *</label>
                                    <select className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.unit_of_measure} onChange={(e) => setForm({ ...form, unit_of_measure: e.target.value })}>
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" className="w-full h-10 px-3 border border-gray-300 rounded-lg text-sm" value={form.default_rate} onChange={(e) => setForm({ ...form, default_rate: e.target.value })} />
                                </div>
                            </div>
