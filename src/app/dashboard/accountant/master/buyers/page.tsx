    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
                ...
                {filterInline && (
                    <span className="text-xs text-slate-500">{displayed.length} of {buyers.length}</span>
                )}
            </div>
                ...
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Brand Code</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Contact</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Shipping Address</th>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {displayed.map((b) => (
                            <tr key={b.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-4 py-3 text-sm text-slate-500 font-mono">{b.brand_code}</td>
                                <td className="px-4 py-3 text-sm text-slate-500">{[b.contact_person, b.phone].filter(Boolean).join(" | ") || "—"}</td>
                                <td className="px-4 py-3 text-sm text-slate-500">{b.shipping_address || "—"}</td>
                                <td className="px-4 py-3 text-sm text-slate-500">{b.notes || "—"}</td>
                            </tr>
                        ))}
                        {displayed.length === 0 && (
                            <tr>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowModal(false)}>
                    <div className="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 p-6" onClick={(e) => e.stopPropagation()}>
                        <h3 className="text-lg font-semibold text-slate-900 mb-4">Add New Buyer</h3>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {error && (
                                <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">{error}</div>
                            )}
                            <div>
                                <label className="block text-xs font-medium text-slate-700 mb-1">Buyer Name *</label>
                            ...
                            <div>
                                <label className="block text-xs font-medium text-slate-700 mb-1">Brand Code *</label>
                                <input type="text" required className="w-full h-10 px-3 border border-slate-300 rounded-lg text-sm uppercase" value={form.brand_code} onChange={(e) => setForm({ ...form, brand_code: e.target.value.toUpperCase() })} />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <textarea rows={2} placeholder="Full shipping address" className="w-full p-3 border border-slate-300 rounded-lg text-sm" value={form.shipping_address} onChange={(e) => setForm({ ...form, shipping_address: e.target.value })} />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-slate-700 mb-1">Notes</label>
                                <textarea rows={2} className="w-full p-3 border border-slate-300 rounded-lg text-sm" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
                            </div>
                            <div className="flex justify-end gap-3 pt-4">
                                <button type="button" onClick={() => { setShowModal(false); setError(""); }} className="px-4 py-2 text-sm text-slate-700 border border-slate-300 rounded-lg hover:bg-slate-50">Cancel</button>
