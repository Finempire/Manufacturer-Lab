...
export default function AccountantTransactionsPage() {
    ...
    return (
        <div className="space-y-4">
            <div>
                <h1 className="text-lg font-semibold tracking-tight text-slate-900">All Transactions</h1>
                <p className="text-sm text-slate-500 mt-1">{payments.length} payments &bull; ₹{totalPaid.toLocaleString("en-IN")} total disbursed</p>
            </div>
                ...
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Invoice</th>
                            <th className="px-4 py-3 text-right text-xs font-semibold text-slate-500 uppercase">Paid</th>
                            <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Ref</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {loading ? (
                            ...
                            <tr key={p.id} className="hover:bg-slate-50">
                                ...
                                <td className="px-4 py-3 text-sm text-slate-500 text-right tabular-nums">₹{p.purchase.invoice_amount.toLocaleString("en-IN")}</td>
