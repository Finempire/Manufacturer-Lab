...
export default function EditOrder() {
    ...
    if (loading) {
        return <div className="flex items-center justify-center py-20 text-slate-400">Loading order...</div>;
    }

    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center gap-3">
                <Link href={`/dashboard/accountant/orders/${orderId}`} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
                    <ArrowLeft className="w-5 h-5" />
                </Link>
                <div>
                    <h1 className="text-lg font-semibold tracking-tight text-slate-900">Edit Order</h1>
                    <p className="text-sm text-slate-500">{orderNo}</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg border border-slate-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" readOnly className="w-full h-9 px-3 bg-slate-100 border border-slate-300 rounded-lg text-sm text-slate-500 cursor-not-allowed" value={orderNo} />
                    </div>
                    <div>
                        ...
                        <table className="min-w-full divide-y divide-slate-200" style={{ tableLayout: "fixed" }}>
                            <thead className="bg-slate-50">
                                <tr>
                                    ...
                                    <th style={{ width: 32 }} className="px-3 py-2 text-center text-xs font-medium text-slate-500 uppercase">x</th>
                                </tr>
                            </thead>
