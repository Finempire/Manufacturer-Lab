...
interface Line { id?: string; style_id: string; description: string; quantity: number; rate: number; amount: number; }
...
export default function EditOrder() {
    ...
    const [formData, setFormData] = useState({
        buyer_id: "", order_date: "", shipping_date: "", order_type: "PRODUCTION", remarks: "",
    });
    ...
    const fetchOrder = useCallback(async () => {
        try {
            ...
            setFormData({
                ...
                shipping_date: new Date(order.shipping_date).toISOString().split("T")[0],
                order_type: order.order_type,
                remarks: order.remarks || "",
            });
            setLines(order.lines.map((l: { id: string; style_id: string; description?: string; quantity: number; rate: number; amount: number }) => ({
                id: l.id,
                style_id: l.style_id,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            router.push(`/dashboard/accountant/orders/${orderId}`);
        } catch (error: unknown) {
    ...
    if (loading) {
        return <div className="flex items-center justify-center py-20 text-foreground-muted">Loading order...</div>;
    }

    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center gap-3">
                <Link href={`/dashboard/accountant/orders/${orderId}`} className="p-2 text-foreground-muted hover:text-foreground-secondary hover:bg-surface-3 rounded-lg transition-colors">
                    <ArrowLeft className="w-5 h-5" />
                </Link>
                <div>
                    <h1 className="text-lg font-semibold tracking-tight text-foreground">Edit Order</h1>
                    <p className="text-sm text-foreground-tertiary">{orderNo}</p>
                </div>
            </div>

            <form onSubmit={handleSubmit} className="space-y-6 bg-surface-1 p-6 rounded-lg border border-border">
                {/* Row 1: Order Type | Buyer */}
                ...
                {/* Row 2: Order No (read-only) | Order Date | Shipping Date */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label className="block text-xs font-medium text-foreground-secondary mb-1">Order No</label>
                        <input type="text" readOnly className="w-full h-9 px-3 bg-surface-3 border border-border rounded-lg text-sm text-foreground-tertiary cursor-not-allowed" value={orderNo} />
                    </div>
                    <div>
                        ...
                        <table className="min-w-full divide-y divide-slate-200" style={{ tableLayout: "fixed" }}>
                            <thead className="bg-surface-2">
                                <tr>
                                    ...
                                    <th style={{ width: 32 }} className="px-3 py-2 text-center text-xs font-medium text-foreground-tertiary uppercase">x</th>
                                </tr>
                            </thead>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <button type="submit" disabled={submitting} className="px-6 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50">{submitting ? "Saving..." : "Save Changes"}</button>
                </div>
            </form>
