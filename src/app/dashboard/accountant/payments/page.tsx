...
export default function AccountantPaymentsPage() {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-lg font-semibold tracking-tight text-slate-900">Process Payments</h1>
                <p className="text-sm text-slate-500 mt-1">Review invoices and upload payment proofs</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                    {purchases.map(pur => (
                        <div key={pur.id} className="bg-white border border-slate-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                            <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-start justify-between">
                                <div>
                                    <h3 className="font-bold text-slate-900">{pur.purchase_no}</h3>
                                    <p className="text-xs text-slate-500 mt-1">Ref: {pur.request?.request_no} • {pur.request?.buyer?.name}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-lg font-bold text-blue-700">₹{pur.invoice_amount.toLocaleString()}</p>
                                    <p className="text-[10px] text-slate-500 uppercase tracking-wider mt-0.5">{pur.invoice_type_submitted}</p>
                                </div>
                            </div>
                            <div className="p-4 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                                <div className="space-y-2">
                                    <div>
                                        ...
                                        <p className="text-sm font-medium text-slate-900">{pur.vendor.name}</p>
                                    </div>
                                    <div className="flex gap-4">
                                        <div>
                                            <p className="text-xs text-slate-500">Invoice No</p>
                                            <p className="text-sm font-medium text-slate-900">{pur.invoice_no}</p>
                                        </div>
                                        <div>
                                            <p className="text-xs text-slate-500">Date</p>
                                            <p className="text-sm font-medium text-slate-900">{format(new Date(pur.invoice_date), "dd MMM yyyy")}</p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <>
                        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
                            <div>
                                ...
                                <p className="text-xs text-slate-500">{selectedPur.purchase_no}</p>
                            </div>
                            ...
                            <form id="payment-form" onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 space-y-3">
                                    <div className="flex justify-between items-center text-sm">
                                        ...
                                        <span className="font-bold text-slate-900">{selectedPur.vendor.name}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <select required className="w-full h-10 px-3 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" value={formData.payment_method} onChange={(e) => setFormData({ ...formData, payment_method: e.target.value })}>
                                    ...
                                    <div>
                                        ...
                                        <input type="date" required className="w-full h-10 px-3 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" value={formData.payment_date} onChange={(e) => setFormData({ ...formData, payment_date: e.target.value })} />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" required min="1" step="0.01" className="w-full h-10 px-3 border border-slate-300 rounded-lg text-sm text-right font-medium tabular-nums focus:ring-2 focus:ring-blue-500 bg-amber-50" value={formData.amount_paid} onChange={(e) => setFormData({ ...formData, amount_paid: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-medium text-slate-700 mb-1">Reference/Txn ID</label>
                                        <input type="text" placeholder="e.g. UTR1234..." className="w-full h-10 px-3 border border-slate-300 rounded-lg text-sm uppercase focus:ring-2 focus:ring-blue-500" value={formData.reference_id} onChange={(e) => setFormData({ ...formData, reference_id: e.target.value })} />
                                    </div>
                                </div>

                                <div>
                                    <label className="block text-xs font-medium text-slate-700 mb-1">Notes (Optional)</label>
                                    <textarea rows={2} className="w-full p-3 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500" placeholder="Any internal notes regarding this payment..." value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} />
                                </div>

                                <div className="pt-2">
                                    <label className="block text-xs font-bold text-slate-900 mb-2">Upload Payment Proof *</label>
                                    <FileUpload
                                        type="PAYMENT_PROOF"
                                        entityId={selectedPur.id}
                                        onUploadSuccess={(path) => setUploadedPath(path)}
                                    />
                                    <p className="text-[10px] text-slate-500 mt-1">Requires screenshot of bank transfer, UPI summary, or photo of signed cheque/cash voucher.</p>
                                </div>
