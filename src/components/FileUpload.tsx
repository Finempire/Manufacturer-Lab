        ...
        <div className="w-full">
            ...
            {status === "idle" && (
                <div
                    ...
                    className="border-2 border-dashed border-gray-300 rounded-lg p-6 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-50 transition-colors bg-white"
                >
                    <UploadCloud className="w-8 h-8 text-gray-400 mb-2" />
                    <p className="text-sm font-medium text-gray-700">Tap to upload or take photo</p>
                    <p className="text-xs text-gray-500 mt-1">Images or PDF up to {maxSizeMB}MB</p>
                </div>
            )}
            ...
            {(status === "selected" || status === "uploading" || status === "error") && file && (
                <div className="border border-gray-200 rounded-lg p-4 bg-white shadow-sm">
                    <div className="flex items-start justify-between">
                        ...
                        {status !== "uploading" && (
                            <button onClick={clearFile} className="p-1 text-gray-400 hover:text-gray-600 rounded">
                                <X className="w-4 h-4" />
                            </button>
                        )}
                    </div>
                    ...
                    {status === "selected" && (
                        <div className="mt-4 flex gap-2">
                            ...
                            <button onClick={() => fileInputRef.current?.click()} className="flex-1 px-3 py-2 text-sm font-medium text-gray-700 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition">
                                Change File
                            </button>
                        </div>
                    )}
